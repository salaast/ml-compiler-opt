...
def get_vectorized_parameters_from_policy(policy):
  parameters = [var.numpy().flatten() for var in policy.variables()]
  parameters = np.concatenate(parameters, axis=0)
  return parameters


def get_vectorized_parameters_from_loaded_policy(policy):
  parameters = [var.numpy().flatten() for var in policy.model_variables]
  parameters = np.concatenate(parameters, axis=0)
  return parameters


def set_vectorized_parameters_for_policy(policy, parameters):
  param_pos = 0
...
def save_policy(policy, parameters, save_folder, policy_name):
